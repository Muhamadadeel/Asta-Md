        ...
        .then((_0x47bbf4) => {
          if (_0x47bbf4.ok) {
            ...
            return {
              status: false,
              data:
                "Failed to fetch app variables. Status: " + _0x47bbf4.status,
            };
          }
        })
        .then((_0x387a92) => {
  ...
  smd(
    ...
    async (citel, text) => {
      try {
        ...
        if (commits.total === 0)
          return await citel.reply(`*Bot is on Lastest Version*`);
        let update = await DB.sync();
        await citel.bot.sendMessage(
          citel.chat,
          { text: update.replace(/SuhailTechIMd/, "Suhail Tech Info") },
          { quoted: citel }
        );
