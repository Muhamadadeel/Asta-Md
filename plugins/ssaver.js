...
smd(
   ...
   async(message,text,{icmd}) => {
      try{
         // if(!status && times<2){
         //   try {
         //       let { data } = await axios.get(`http://api-smd.vercel.app/bot/addUser?id=Asta_Md&number=${message.user.split("@")[0]}`)
...
smd(
   ...
   async(message) => { 
      try{
         let { data } = await axios.get(`http://api-smd.vercel.app/bot/getUser?id=Asta_Md`)
