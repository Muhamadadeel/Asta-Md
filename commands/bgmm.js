...
async function convertAudioToBlackScreenVideo(audioPath, outputPath) {
  try {
    ...
    const {
      stdout: durationOutput
    } = await exec(getDurationCmd);
...
cmd({
  ...
  if (bgmm.bgmArray.has(text)) {
    ...
    return await citel.reply(`Name _'${text}'_ does not exist in BGM.`);
  }
})
cmd({
  pattern: "allbgm",
...
cmd({
  ...
  try {
    ...
    return await citel.reply('Error updating BGM:' + error);
  }
})
cmd({
  on: "text"
    ...
    for (const [name, url] of bgmm.bgmArray) {
      ...
      if (citelText.toLowerCase()
        ...
        return await Void.sendMessage(citel.chat, {
          ...
          mimetype: 'audio/mpeg',
          ptt: true,
          waveform: [99, 75, 25, 00, 00, 00, 00, 00, 00, 00, 00, 00, 05, 25, 50, 75, 99, 75, 50, 25, 00]
        })
      }
    }
  }
