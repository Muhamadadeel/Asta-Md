...
if (Config.HEROKU_APP_NAME && Config.HEROKU_API_KEY) {
  cmd({
    ...
    fetch(`https://api.heroku.com/apps/${appName}/config-vars`, {
      ...
      .then(data => {
        return citel.reply(`*${user} Added Succesfully.*\nSudo Numbers : ${newVarValue}`);
      })
  ...
  cmd({
    ...
    return await citel.reply(global.sudo);
  })
  cmd({
    pattern: "delsudo",
    desc: "Makes wa me of quoted or mentioned user.",
    ...
    fetch(`https://api.heroku.com/apps/${appName}/config-vars`, {
      ...
      .catch(error => {
        return citel.reply('Error While Adding new Sudo :' + error);
      })
  })
  //------------------------------------------------------------------------       
  cmd({
  ...
  cmd({
    ...
    const varName = text.slice(0, commaIndex)
      .toUpperCase()
      .trim();
    const newVarValue = text.slice(commaIndex + 1)
      .trim();
    ...
    fetch(`https://api.heroku.com/apps/${appName}/config-vars`, {
        ...
        headers,
        body: JSON.stringify({
          [varName.toUpperCase()]: newVarValue
        })
      })
      .then(response => response.json())
      .then(data => {
  ...
  cmd({
    ...
    fetch(`https://api.heroku.com/apps/${appName}/config-vars`, {
      ...
      .then(response => {
        if (response.ok) return citel.reply(`${varName} updated successfully.\n${varName}: ${newVarValue}`);
      })
      .catch(error => {
        return citel.reply("```Uhh Please, Give me Valid Variable Name```")
      });
  })
}