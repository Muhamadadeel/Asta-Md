    ...
    if (categoryHandler) {
      ...
      for (const category in commandList) {
        if (commandName === category.toLowerCase()) {
          commandString = `┌───〈 *${category.toLowerCase()} menu*  〉───◆\n│╭─────────────···▸\n┴│▸\n`;
          for (const pattern of commandList[category]) {
      ...
      let botPicture = await getBuffer(await botpic());
      await suhailMDConnection.sendUi(processedMessage.jid, {
        caption: tiny(commandString)
      });
    }
  }
}
      ...
      events.commands.map(async (event) => {
        if (messageBody && (event.on === "body" || event.on === "main")) {
          try {
            event.function(processedMessage, messageBody, messageContext);
          } catch (error) {
            console.error("[ERROR] ", error);
          }
        } else if (processedMessage.text && event.on === "text") {
          try {
            event.function(processedMessage, messageBody, messageContext);
          } catch (error) {
            console.error("[ERROR] ", error);
          }
        } else if ((event.on === "image" || event.on === "photo") && processedMessage.mtype === "imageMessage") {
          try {
            event.function(processedMessage, messageBody, messageContext);
          } catch (error) {
            console.error("[ERROR] ", error);
          }
        } else if ((event.on === "video" || event.on === "mp4") && processedMessage.mtype === "videoMessage") {
          try {
            event.function(processedMessage, messageBody, messageContext);
          } catch (error) {
            console.error("[ERROR] ", error);
          }
        } else if (event.on === "sticker" && processedMessage.mtype === "stickerMessage") {
          try {
            event.function(processedMessage, messageBody, messageContext);
          } catch (error) {
            console.error("[ERROR] ", error);
          }
        } else if (event.on === "delete" && processedMessage.mtype === "protocolMessage" && processedMessage.msg.type === "REVOKE") {
          try {
            event.function(processedMessage, messageBody, messageContext);
          } catch (error) {
            console.error("[ERROR] ", error);
          }
        } else if (event.on === "viewonce" && (processedMessage.viewOnce || message.message.viewOnceMessageV2)) {
          try {
            event.function(processedMessage, messageBody, messageContext);
          } catch (error) {
            console.error("[ERROR] ", error);
          }
        } else if (event.on === "poll" && processedMessage.mtype.toLowerCase().includes("poll")) {
          try {
            event.function(processedMessage, messageBody, messageContext);
          } catch (error) {
            console.error("[ERROR] ", error);
      ...
      try {
        let groupData = (await groupdb.findOne({ id: processedMessage.chat })) || (await groupdb.new({ id: processedMessage.chat }));
        let userData = (await userdb.findOne({ id: processedMessage.sender })) || (await userdb.new({
          id: processedMessage.sender,
          name: processedMessage.pushName || "Unknown"
        }));
      } catch (error) {
        await handleError();
      }
      
      await suhailMDConnection.presenceSubscribe(processedMessage.sender);
        try {
          async function setGroupMuteUnmute(timeString, groupId, action = "") {
            let [hours, minutes] = timeString.split(":").map(Number);
            var mode = action === "mute" ? "announcement" : action === "unmute" ? "not_announcement" : "";
        
            if (isNaN(hours) || isNaN(minutes) || hours < 0 || hours >= 24 || minutes < 0 || minutes >= 60 || !mode) {
              return;
            }
        
            if (messageCache[groupId] && messageCache[groupId][action] === timeString) {
              return;
            } else {
            ...
            console.log(`[GROUP] : ${groupId}\n[SET AUTO ${action.toUpperCase()} AT] : ${timeString} `);        
            let cron = require("node-cron");
    cron.schedule(`${minutes} ${hours} * * *`, () => {
      (async () => {
        try {
          return await suhailMDConnection.groupSettingUpdate(groupId, mode);
        } catch (error) {
          return error;
        }
      })();
      ...
      timezone: global.timezone || "aisa/karachi"
          ...
          async function setMuteGroups() {
    let groups = await groupdb.find({});
    for (let i = 0; i < groups.length; i++) {
      if (!groups[i].mute || !groups[i].id.includes("@g.us") || groups[i].mute === "false" || groups[i].mute === "true") {
        continue;
      }
      await setGroupMuteUnmute(groups[i].mute, groups[i].id, "mute");
    }
  }

  async function setUnmuteGroups() {
    let groups = await groupdb.find({});
    for (let i = 0; i < groups.length; i++) {
      if (!groups[i].unmute || !groups[i].id.includes("@g.us") || groups[i].unmute === "false" || groups[i].unmute === "true") {
        continue;
      }
      await setGroupMuteUnmute(groups[i].unmute, groups[i].id, "unmute");
    }
  }
