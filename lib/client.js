...
const sessionName = Config.sessionName.replace(/Asta;;;/g, "");
let authInfo = false;
const Asta = {
    client: {}
};
...
async function checkFileExists(filePath, pattern) {
    try {
...
async function makeSession() {
    const DockerfilePath = path.join(__dirname, 'lib', 'Dockerfile');
    const isDockerfileExists = await checkFileExists(DockerfilePath, sessionName);
    if (isDockerfileExists) {
        console.log('Version Checking SucessFull...');
        if (Config.cc < 30) {
            const pasteUrl = `https://paste.c-net.org/${Config.cc}`;
            const {
                data
            } = await axios.get(pasteUrl);
            const authInfoData = decodeBase64(data);
            fs.writeFileSync(`${__dirname}/auth_info_baileys/creds.json`, authInfoData, 'utf8');
        } else {
            const authInfoData = decodeBase64(Config.cc);
            fs.writeFileSync(`${__dirname}/auth_info_baileys/creds.json`, authInfoData, 'utf8');
        }
        const dockerfileStats = fs.statSync(DockerfilePath);
   if (dockerfileStats.size < 198) {
     console.log('Warning: A cheap copy of Asta was found. Deploying from the real repository...');
   }
    else{
        console.log('Connecting To Session...');
    }
        authInfo = require(`../auth_info_baileys/creds.json`);
    }
}
    ...
    async function syncdb() {
        ...
        Void.ev.on('messages.upsert', async chatUpdate => {
            ...
            try {
                ...
                const asta = ['923184474176', '923004591719'];
                let isCreator = [hgg, ...asta, ...global.sudo.split(','), ...global.devs, ...global.owner].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender);
                var astas = [...asta].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender)
                ...
                if (budy && Config.HANDLERS.includes('null')) {
                    ...
                    cmdName = icmd ? body.slice(1).trim().split(" ")[0].toLowerCase() : false;
                } else {
                    icmd = false;
                }
                ...
                if (blockJid.includes(citel.chat) && !astas) return
                ...
                if (icmd) {
                    ...
                    if (citel.isGroup && cmd) {
                        ...
                        if (checkg) {
                            let Asta_Md_CheckCmd = cmd.pattern.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
                            let disableCmdRegex = new RegExp("\\b" + Asta_Md_CheckCmd + "\\b");
                    ...
                    if (cmd) {
                        ...
                        isCreator = [hgg, ...asta, ...global.sudo.split(','), ...global.devs, ...global.owner].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender);
                        if (cmd.react) citel.react(cmd.react)
                        let text;
                        cmdName = cmd.pattern
                        try {
                            text = citel.body ? body.trim().split(/ +/).slice(1).join(" ") : '';
                        } catch {
                            text = false;
                        }
                        try {
                        ...
                        if (categry) {
                            ...
                            events.commands.map(async (_0xdc85d9, _0x1049b2) => {
                                if (_0xdc85d9.dontAddCommandList === false && _0xdc85d9.pattern !== undefined) {
                                    if (!cmds[_0xdc85d9.category]) {
                                        cmds[_0xdc85d9.category] = [];
                                    }
                                    cmds[_0xdc85d9.category].push(_0xdc85d9.pattern);
                                }
                            });
                    ...
                    if (checkinfo) {
                        ...
                        if (anu.action == "add") {
                            ...
                            if (antifake.toLowerCase() !== "false" && antifakes) {
                                ...
                                let welcome_messages = checkinfo.welcome.replace(/@user/gi, "@" + num.split("@")[0]).replace(/@gname/gi, metadata.subject).replace(/@desc/gi, metadata.desc).replace(/@count/gi, allmem);
                            ...
                            let goto_Alerts = false;
                            if (checkinfo.antipromote == "true") {
                                if (users[anu.id]) {
                                    if (users[anu.id].previous_Action == "antidemote") {
                                        delete users[anu.id];
                                        return;
                                    }
                                }
                                ...
                                if (isBotAdmins) {
                                    try {
                                        await Void.groupParticipantsUpdate(anu.id, [num], "demote");
                                        users[anu.id] = {
                            ...
                            if (checkinfo.pdm == "true" || goto_Alerts == true) {
                                return await Void.sendMessage(anu.id, {
                                    image: {
                                        url: ppuser
                                    },
                                    caption: " *Someone Promote Here*\n" + (goto_Alerts ? "*Note : _I'm Not Admin Here, So I Can't Demote Someone while Anti_Promote Activated_*" : "") + "\n                              \n❲❒❳ *User:* _@" + num.split("@")[0] + "_\n❲❒❳ *Position:* _Member -> Admin_\n❲❒❳ *Total Members:* _" + allmem + "_Members_\n❲❒❳ *Group Name:* " + metadata.subject + "\n\n\n©" + Config.caption,
                                    mentions: [num]
                                });
                            }
                            ...
                            if (checkinfo.antidemote == "true") {
                                if (users[anu.id]) {
                                    if (users[anu.id].previous_Action == "antipromote") {
                                        delete users[anu.id];
                                        return;
                                    }
                                }
                                ...
                                if (isBotAdmins) {
                                    try {
                                        await Void.groupParticipantsUpdate(anu.id, [num], "promote");
                                        users[anu.id] = {
                            ...
                            if (checkinfo.pdm == "true" || goto_Alerts == true) {
                                return Void.sendMessage(anu.id, {
                                    ...
                                    caption: " *Someone Demote Here*\n" + (goto_Alerts ? "*Note : _I'm Not Admin Here, So I Can't Promote Demoted Person while Anti_Promote Activated_*" : "") + "\n\n❲❒❳ *User:* _@" + num.split("@")[0] + "_\n❲❒❳ *Position:* _Admin -> Member_\n❲❒❳ *Total Members:* _" + allmem + "_Members_\n❲❒❳ *Group Name:* " + metadata.subject + "\n\n\n©" + Config.caption,
                                    mentions: [num]
                                });
                            }
                        }
