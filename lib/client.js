...
let cc = Config.sessionName.replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/, "").replace(/Asta;;;/gi, "").replace(/Asta_Bot;;;/gi, "").replace(/Astro;;;/gi, "").replace(/^SUHAIL_\d{2}\d{2}\d{2}\d{2}/, "").trim();
var astros = false;
...
async function createSession() {
  ...
  const expectedContent = "/SuhailTechInfo/";
  ...
  if (isValidVersion) {
    ...
    if (cc && cc.length > 3 && cc.length < 30) {
      const axios = require("axios");
      try {
        let { data: sessionData } = await axios.get("https://paste.c-net.org/" + cc);
...
setTimeout(() => {
  ...
  let connectionStatus = "invalid";
const messageRetryMap = MessageRetryMap || {};

async function setupBot() {
 let thumbImagePath = __dirname + "/assets/suhail.jpg";
 try {
   let thumbBuffer = await getBuffer(THUMB_IMAGE);
   thumbImagePath = __dirname + "/assets/SocialLogo.png";
   await writeFile(thumbImagePath, thumbBuffer);
 } catch (error) {
   thumbImagePath = __dirname + "/assets/suhail.jpg";
 }
 global.log0 = fs.readFileSync(thumbImagePath);

 const { state, saveCreds } = await useMultiFileAuthState(__dirname + baileys);
 const bot = SuhailMDConnect({
   logger: Pino({
     level: "silent" || "debug"
   }).child({
     level: "silent"
   ...
   getMessage: async (message) => {
     if (store) {
       const storedMessage = await store.loadMessage(message.remoteJid, message.id, undefined);
       return storedMessage.message || undefined;
       console.log("getMessage: ", storedMessage);
     }
     return {
 ...
 bot.ev.on("call", async (call) => {
   let callData = await callsg(bot, JSON.parse(JSON.stringify(call[0])));
   events.commands.map(async (commandData) => {
     if (commandData.call === "offer" && callData.status === "offer") {
       try {
         commandData.function(callData, {
           store: store,
           bot: bot
         });
       } catch (error) {
     ...
     if (commandData.call === "accept" && callData.status === "accept") {
       try {
         commandData.function(callData, {
           store: store,
           bot: bot
     ...
     if (commandData.call === "call" || commandData.call === "on" || commandData.call === "all") {
       try {
         commandData.function(callData, {
           store: store,
           bot: bot
