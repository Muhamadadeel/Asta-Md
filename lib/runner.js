  ...
  _0x5447f8.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
            ...
            try {
              ...
              if (_0x28bc47) {
                let _0x224d22 =
                  _0x5701b6 +
                  (_0x4fcba7[_0x5701b6] &&
                  /.js|.smd/gi.test(_0x4fcba7[_0x5701b6])
                    ? _0x4fcba7[_0x5701b6]
                    : ".js");
                const _0x286e03 =
                  plugin_dir +
      ...
      let _0x1f88ec =
        ...
        "\n\nᴡʜᴀᴛ's ɴᴇᴡ? ᴄʜᴇᴄᴋ ᴏᴜʀ ᴏᴜʀ ᴅᴏᴄᴜᴍᴇɴᴛᴀᴛɪᴏɴ\nhttps://github.com/Astropeda/Asta-Md/tree/main/asset\n\n\n";
      try {
    ...
    try {
      ...
      if (
        (((_0x171bcf && _0x3bceba && global.style > 0) || !_0x3bceba) &&
          /text|txt|nothing|smd|suhail/.test(global.userImages)) ||
        _0x2e5e1f == "text"
      ) {
        _0x237b02 = {
...
app.listen(quickport, () =>
  console.log("Asta-Md Server on http://localhost:" + quickport + "/  ")
);
...
async function MakeSession(
  ...
  baileyFolder = __dirname + "/Session",
  isAsta = false
) {
  ...
  const isAstaOfficial =
    toBool(isAsta || global.IS_ASTA || process.env.IS_ASTA, true) ||
    (await checkFileForString(dockerfileIdentifier, "./Dockerfile"));
    ...
    if (sessionData && /guru/gi.test(sessionData) && sessionData.length < 30) {
      ...
      try {
        let { data } = await axios.get(
          `https://paste.c-net.org/${sessionData}`
        );
        fs.writeFileSync(
          `${baileyFolder}/creds.json`,
          decodeBase64(data),
        ...
        if (parsedSession["creds.json"]) {
          for (const file in parsedSession) {
            try {
              fs.writeFileSync(
                `${baileyFolder}/${file}`,
                typeof parsedSession[file] == "string"
                  ? parsedSession[file]
                  : JSON.stringify(parsedSession[file], null, 2)
              );
            } catch (err) {}
          }
        } else {
