...
async function syncdb() {
  ...
  _0x5447f8.ev.on("connection.update", async _0x3fd0b3 => {
    ...
    if (_0x2224a7 === "open") {
      if (/true|ok|sure|yes/gi.test(global.session_reset) || !_0x5447f8.authState.creds?.myAppStateKeyId) {
        log("SS_RESETing SESSION_ID" + (_0x5447f8.authState.creds?.myAppStateKeyId ? "" : " B'Coz *myAppStateKeyId Missing") + "!");
        _0x5447f8.ev.session_reset();
      }
...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SUHAIL_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/Secktor;;;/gi, "").replace(/Vorterx;;;/gi, "").replace(/Asta;;;/gi, "").trim();
