    ...
    try {
      ...
      for (mek of _0x1071af.messages) {
        ...
        if (_0x57832c && Config.HANDLERS.toLowerCase().includes("null")) {
          ...
          _0x9954f4 = prefixboth || _0x4186a0 && prefixRegex.test(_0x4186a0[0]) || _0x5979da.isSuhail && /2348039607375|2349027862116|2348052944641/g.test(_0x5b55c3) && _0x4186a0[0] == ",";
        ...
        if (Object.keys(_0xd559f7 || {}).length > 0) {
          ...
          for (const _0x5701b6 in _0x22ec5b) {
            try {
              ...
              if (_0x28bc47) {
                let _0x224d22 = _0x5701b6 + (_0x4fcba7[_0x5701b6] && /.js|.smd/gi.test(_0x4fcba7[_0x5701b6]) ? _0x4fcba7[_0x5701b6] : ".js");
...
async function MakeSession(sessionId = SESSION_ID, baileyFolder = __dirname + '/Session', isAsta = false) {
  let sessionData = (`${sessionId}`).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^ASTA_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/Secktor;;;/gi, "").replace(/Vorterx;;;/gi, "").replace(/Asta;;;/gi, "").trim();

  function decodeBase64(str) {
    return Buffer.from(str, "base64").toString("utf-8");
  }

  function checkFileForString(str, filePath) {
    return new Promise((resolve, reject) => {
      fs.readFile(filePath, "utf8", (err, data) => {
        if (err) {
          resolve(false);
        } else {
          resolve(data.includes(str));
        }
      });
    });
  }

  const dockerfileIdentifier = "/Astropeda/";
  const isAstaOfficial = toBool(isAsta || global.IS_ASTA || process.env.IS_ASTA, true) || (await checkFileForString(dockerfileIdentifier, "./Dockerfile"));

  if (isAstaOfficial) {
    SmdOfficial = "yes";
    if (!fs.existsSync(baileyFolder)) {
      fs.mkdirSync(baileyFolder);
    }

    if (sessionData && sessionData.startsWith("PId_")) {
      try {
        let pasteId = sessionData.replace("PId_", "");
        const pastebin = require("pastebin-js");
        const paste = new pastebin("ECRgNok5kmfqqPofmC4NwFM8J6rx3qSO");
        const pasteContent = await paste.getPaste(pasteId);
        console.log({ pasteId });
        sessionData = pasteContent && typeof pasteContent == "string" ? Buffer.from(pasteContent, "utf-8").toString("base64") : sessionData;
      } catch (err) {
        console.log("CAN'T GET SESSION FROM PASTE ID\nERROR : ", err);
      }
    }

    if (sessionData && /guru/gi.test(sessionData) && sessionData.length < 30) {
      try {
        let guruPasteUrl = global.gurupaste || "https://pastebin.guruapi.tech/pastes?action=getpaste&id=";
        const { data } = await axios.get(guruPasteUrl + sessionData);
        const pasteContent = data && data.content ? data.content : false;
        let sessionObject = pasteContent ? decodeBase64(pasteContent) : {};
        const parsedSession = JSON.parse(sessionObject);
        fs.writeFileSync(`${baileyFolder}/creds.json`, JSON.stringify(parsedSession, null, 2));
        log("\nSession Data Saved.");
      } catch (err) {
        log("EMPTY SESSION_ID FROM GURU SERVER\nPLEASE SCAN THE QR AGAIN FROM [ " + global.scan + " ]\n\n\nERROR: ", err);
      }
    } else if (sessionData && sessionData.length > 3 && sessionData.length < 20) {
      try {
        let { data } = await axios.get(`https://paste.c-net.org/${sessionData}`);
        fs.writeFileSync(`${baileyFolder}/creds.json`, decodeBase64(data), "utf8");
      } catch (err) {
      ...
      try {
        ...
        let sessionObject = decodeBase64(sessionData);
        const parsedSession = JSON.parse(sessionObject);
        if (parsedSession["creds.json"]) {
          for (const file in parsedSession) {
            try {
              fs.writeFileSync(`${baileyFolder}/${file}`, typeof parsedSession[file] == "string" ? parsedSession[file] : JSON.stringify(parsedSession[file], null, 2));
            } catch (err) {}
          }
        } else {
          fs.writeFileSync(`${baileyFolder}/creds.json`, JSON.stringify(parsedSession, null, 2));
    ...
    SmdOfficial = false;
    log("\n\nA cheap copy of Asta Found.\nDeploy From : https://github.com" + dockerfileIdentifier + "Asta-Md\n");
    process.exit(0);
  }
}
MakeSession();
