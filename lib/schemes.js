let options = {
  ...
  tempdb: "creator TEXT DEFAULT 'Asta-SIR'"
};
let optJson = {
  bot_: {},
  sck1: {
...
pg.ConnectPostgreSQL = () => {
  ...
  pool.on("error", _0x18b37d => {
    ...
    setTimeout(pg.ConnectPostgreSQL, 1000);
  });
};
pg.createTable = async _0x2100aa => {
  if (!sqldb && !cacheTable.ConnectPostgreSQL || !pool && global.sqldb) {
    let _0x491b2b = pg.ConnectPostgreSQL();
    if (!_0x491b2b) {
      return false;
    }
  }
