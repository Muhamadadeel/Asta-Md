...
setInterval(() => {
      ...
      for (let mek of messageEvent.messages) {
        ...
        if (validCommand) {
          ...
          if (messageObject.isGroup && matchedCommand && commandTrigger !== "bot") {
            let groupConfig = _0x4f1890[messageObject.chat] || (await groupdb.findOne({ id: messageObject.chat })) || {
          ...
          if (!isCreator && matchedCommand) {
            try {
              let userConfig = _0x686f61[messageObject.sender] || (await userdb.findOne({ id: messageObject.sender })) || { ban: "false" };
  ...
  Sock.serializeM = (_0x40f3f1) =>
    smsg(Sock, _0x40f3f1, store, false);
