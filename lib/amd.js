...
const { userdb, groupdb, bot_, smdBuffer } = require("../lib");
...
const connnectpg = async () => {
  try {
    ...
    const pgconnect = await _0x39ea68.connect();
    pgconnect.release();
...
const connnectMongo = async () => {
  ...
  try {
    mongoconnect.set("strictQuery", true);
...
async function syncdb() {
  let thumbnail = __dirname + "/assets/logo.jpeg";
  try {
    global.log0 =
      typeof THUMB_IMAGE === "string"
        ? await getBuffer(THUMB_IMAGE.split(",")[0])
        : fs.readFileSync(thumbnail);
  } catch (err) {
    thumbnail = __dirname + "/assets/logo.jpeg";
  }
  global.log0 = global.log0 || fs.readFileSync(thumbnail);
  const { state: creds, saveCreds: SavedCreds } = await useMultiFileAuthState(
    __dirname + baileys
  );
  let sock = BotConnect({
    logger: Pino({
    ...
    auth: creds,
    getMessage: async (jids) => {
      let chats = {
        conversation: "Bot",
      };
      if (store) {
        const _0x27b3c1 = await store.loadMessage(jids.remoteJid, jids.id);
        return _0x27b3c1.message || chats;
      }
      return chats;
    },
  });
  ...
  sock.ev.on("call", async (call_func) => {
    let current = await callsg(sock, JSON.parse(JSON.stringify(call_func[0])));
    events.commands.map(async (socket) => {
      if (socket.call === "offer" && current.status === "offer") {
        try {
          socket.function(current, {
            store: store,
            Void: sock,
          });
        } catch (err) {
      ...
      if (socket.call === "accept" && current.status === "accept") {
        try {
          socket.function(current, {
            store: store,
            Void: sock,
  ...
  sock.ev.on("creds.update", SavedCreds);
  ...
  sock.contextInfo = async (
    ...
    try {
      let choice = mtype ? mtype : global.style;
      if (choice >= 5) {
        return {
          externalAdReply: {
            title: BOTNAME,
...
global.toBool = (foo, choo = false) =>
  /true|yes|ok|act|sure|enable|amd/gi.test(foo)
    ? choo
      ? true
      : "true"
    : choo
    ? false
    : "false";
async function loadPlugins(plugins_path) {
  try {
    fs.readdirSync(plugins_path).forEach((files) => {
      const dir = path.join(plugins_path, files);
      if (fs.statSync(dir).isDirectory()) {
        loadPlugins(dir);
      } else if (files.includes("_Baileys") || files.includes("_MSGS")) {
        log(
          "\nSHAREBOT DATA DETECTED!",
          "\nUSER NUMBER:",
          files.replace("_MSGS", "").replace("_Baileys", ""),
          "\n\n"
        );
      } else if ([".js", ".pak"].includes(path.extname(files).toLowerCase())) {
        try {
          require(dir);
        } catch (error) {
          log("\n❌There's an error in '" + files + "' file ❌ \n\n", error);
        }
      }
    });
  } catch (err) {}
...
<html>
<head>
    <title>Astropeda GitHub Page</title>
    <style>
        body {
            margin: 0;
            padding: 0;
...
app.get("/qr", async (_0x1e3486, session_pas) => {
  try {
    if (!global.qr) {
      throw "QR NOT FETCHED!";
    }
    let qr_scan = require("qrcode");
    session_pas.end(await qr_scan.toBuffer(global.qr));
  } catch (err) {
    console.log("/qr PATH_URL Error : ", err);
    if (!session_pas.headersSent) {
      session_pas.send({
...
function keepAlive() {
  setInterval(() => {
    for (let aliver = 0; aliver < Appurls.length; aliver++) {
      const server_info = Appurls[aliver];
      if (/(\/\/|\.)undefined\./.test(server_info)) {
        continue;
      }
      try {
        axios.get(server_info);
      } catch (_0x231f6e) {}
      try {
        fetch(server_info);
      } catch (err) {}
...
async function MakeSession(
  _0x3344dc = SESSION_ID,
  dir = __dirname + baileys,
  inspect = false
) {
  let SESSION_ATOB = ("" + _0x3344dc)
    ...
    .trim();
  function process(SESSION_DATA) {
    return Buffer.from(SESSION_DATA, "base64").toString("utf-8");
  }
  function vaildate(match, docker) {
    return new Promise((d_path, version_pak) => {
      fs.readFile(docker, "utf8", (data, qued) => {
        if (data) {
          d_path(false);
        } else {
          d_path(qued.includes(match));
        }
      });
    });
  }
  const Owner = "/Astropeda/";
  const DockerFilePath =
    toBool(inspect || global.IS_TRUE || process.env.IS_TRUE, true) ||
    (await vaildate(Owner, "./assets/Dockerfile"));
  if (DockerFilePath) {
    SmdOfficial = "yes";
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir);
    }
    if (SESSION_ATOB && SESSION_ATOB.startsWith("PId_")) {
      try {
        var createpasta = SESSION_ATOB.replace("PId_", "");
        const url = require("pastebin-js");
        const api_key = new url("ECRgNok5kmfqqPofmC4NwFM8J6rx3qSO");
        const convertion = await api_key.getPaste(createpasta);
        console.log({
          pasteId: createpasta,
        });
        SESSION_ATOB =
          convertion && typeof convertion == "string"
            ? Buffer.from(convertion, "utf-8").toString("base64")
            : SESSION_ATOB;
      } catch (err) {
    ...
    if (
      SESSION_ATOB &&
      /guru/gi.test(SESSION_ATOB) &&
      SESSION_ATOB.length < 30
    ) {
      try {
        let SESSION_ATOBB =
          global.gurupaste ||
          "https://pastebin.guruapi.tech/pastes?action=getpaste&id=";
        const { data: compressed } = await axios.get(
          SESSION_ATOBB + SESSION_ATOB
        );
        const output =
          compressed && compressed.content ? compressed.content : false;
        var result = output ? process(output) : {};
        const pasredlizedToFile = JSON.parse(result);
        fs.writeFileSync(
          dir + "creds.json",
          JSON.stringify(pasredlizedToFile, null, 2)
        );
      ...
      SESSION_ATOB &&
      SESSION_ATOB.length > 3 &&
      SESSION_ATOB.length < 20
    ) {
      try {
        let { data: ss_data } = await axios.get(
          "https://paste.c-net.org/" + SESSION_ATOB
        );
        fs.writeFileSync(dir + "creds.json", process(ss_data), "utf8");
      } catch (err) {
        log(
    ...
    } else if (SESSION_ATOB) {
      try {
        log("Checking Session ID!");
        var result = process(SESSION_ATOB);
        const init = JSON.parse(result);
        if (init["creds.json"]) {
          for (const data in init) {
            try {
              fs.writeFileSync(
                dir + data,
                typeof init[data] == "string"
                  ? init[data]
                  : JSON.stringify(init[data], null, 2)
              );
            } catch (_0x1d9fd6) {}
          }
        } else {
          fs.writeFileSync(dir + "creds.json", JSON.stringify(init, null, 2));
        }
        log("\nCredentials Saved Successfully.");
      } catch (err) {
        log(
