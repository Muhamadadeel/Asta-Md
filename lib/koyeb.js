...
async function get_deployments() {
  ...
  await axios.get(`https://app.koyeb.com/v1/deployments`, axiosConfig)
    .then(res => {
...
async function delvar(a) {
  ...
  await axios.patch(`https://app.koyeb.com/v1/services/${ser}`, body, axiosConfig)
    .then(res => {
      if (res.status === 200) {
        status = `_Successfully deleted ${a} var from koyeb._`
      } else {
