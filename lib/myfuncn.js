...
exports.generateMessageTag = (epoch) => {
  let tag = (0, exports.unixTimestampSeconds)()
    .toString();
  if (epoch) tag += '.--' + epoch; // attach epoch if provided
  return tag;
}
exports.processTime = (timestamp, now) => {
  return moment.duration(now - moment(timestamp * 1000))
    .asSeconds()
}
...
exports.generateProfilePicture = async (buffer) => {
  ...
  return {
    img: await cropped.scaleToFit(720, 720)
      .getBufferAsync(jimp_1.MIME_JPEG),
    preview: await cropped.scaleToFit(720, 720)
      .getBufferAsync(jimp_1.MIME_JPEG)
  }
}
...
exports.smsg = (conn, citel, store) => {
  ...
  if (citel.message) {
    ...
    if (citel.quoted) {
      ...
      if (quoted.viewOnceMessageV2) {
        ...
        citel.quoted.delete = async () => await conn.sendMessage(citel.chat, {
          delete: key
        })
        /**
         * 
         ...
         * @returns 
         */
        citel.forwardMessage = (jid, forceForward = true, options = {}) => conn.copyNForward(jid, vM, forceForward, {
          contextInfo: {
            isForwarded: false
          }
        }, options)
  ...
  citel.sticker = (stik, id = citel.chat, option = {
  ...
  citel.replyimg = (img, teks, id = citel.chat, option = {
    mentions: [citel.sender]
  }) => conn.sendMessage(id, {
    image: img,
    caption: teks,
  ...
  citel.imgurl = (img, teks, id = citel.chat, option = {
    mentions: [citel.sender]
  }) => conn.sendMessage(id, {
    image: {
      url: img
  ...
  citel.send = async (content, opt = {
    packname: "Asta"
  }, type = "text") => {
    switch (type.toLowerCase()) {
    case "text": {
      return await conn.sendMessage(citel.chat, {
        text: content,
        ...opt
      });
    }
    ...
    case "image": {
      if (Buffer.isBuffer(content)) {
        ...
        return conn.sendMessage(citel.chat, {
          image: {
            url: content
          },
          ...opt
  ...
  citel.senddoc = (doc, type, id = citel.chat, option = {
    mentions: [citel.sender],
  ...
  citel.react = (emoji) => conn.sendMessage(citel.chat, {
    react: {
      text: emoji,
      key: citel.key
    }
