...
async function forwardMessage(r, u, H = "") {
  ...
  if (z) {
    try {
      await Suhail.bot.relayMessage(r, z, {
        messageId: u.key.id
      });
    } catch (h) {
      if (H === "ptv" || H === "save") {
...
async function audioEditor(r, u = "bass", H = "") {
  ...
  try {
    ...
    let j = "temp/" + (r.sender.slice(6) + u) + ".mp3";
