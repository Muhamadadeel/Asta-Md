/*
This is a module that exports various utility functions for working with messages in a chat application, specifically using the @sampandey001/baileys library for WhatsApp Web. Here are some of the key functions:

unixTimestampSeconds: returns the current Unix timestamp in seconds
generateMessageTag: generates a unique message tag based on the current Unix timestamp in seconds
processTime: calculates the time difference between two Unix timestamps in seconds
getBuffer: fetches a file from a URL and returns it as a Buffer
fetchJson: fetches JSON data from a URL and returns it
runtime: converts a number of seconds to a formatted string with days, hours, minutes, and seconds
clockString: converts a number of seconds to a formatted string in the format of "HH:MM:SS"
sleep: pauses execution for a specified number of milliseconds
isUrl: checks if a string is a valid URL
getTime: formats a date object or timestamp to a string using the specified format and locale
formatDate: formats a date object to a string with the full date and time
formatp: formats a number of bytes to a string with a unit suffix (e.g. "10 KB")
jsonformat: formats a JavaScript object to a formatted JSON string
logic: returns a value from an array based on a given condition
generateProfilePicture: resizes and crops an image to a square and returns it as a Buffer
bytesToSize: converts a number of bytes to a string with a unit suffix (e.g. "10 KB")
getSizeMedia: returns the size of a media file as a string with a unit suffix
parseMention: extracts mentioned user IDs from a message object
GIFBufferToVideoBuffer: converts a GIF Buffer to a video Buffer
smsg: serializes a message object and adds metadata such as the sender, chat, and timestamp
The module also includes functions for forwarding messages, downloading media, replying to messages, sending stickers, and sending documents or contacts. The module uses several external libraries, including axios, cheerio, fs-extra, human-readable, moment-timezone, jimp, and fluent-ffmpeg.
...
if (fs.existsSync('./sources/' + Config.LANG + '.json')) {var json = JSON.parse(fs.readFileSync('./sources/' + Config.LANG + '.json'));} 
else {	var json = JSON.parse(fs.readFileSync('./sources/ASTA.json'));	}	
