...
function pinterest(query) {
  return new Promise(async (message, _0x50133a) => {
    axios.get("https://id.pinterest.com/search/pins/?autologin=true&q=" + query, {
      ...
      const QuerySearch = cheerio.load(result);
      const Init = [];
      const DeInit = [];
      QuerySearch("div > a").get().map(Results => {
        const FullData = QuerySearch(Results).find("img").attr("src");
        Init.push(FullData);
      });
      Init.forEach(Search => {
        if (Search == undefined) {
        ...
        DeInit.push(Search.replace(/236/g, "736"));
      });
      DeInit.shift();
      message(DeInit);
    });
...
function TelegraPh(query) {
  return new Promise(async (message, result) => {
    if (!fs.existsSync(query)) {
      return result(new Error("File not Found"));
    }
    try {
      const GetData = new BodyForm();
      GetData.append("file", fs.createReadStream(query));
      ...
      return message("https://telegra.ph" + _0x5dd8d3.data[0].src);
    } catch (err) {
      return result(new Error(String(err)));
    }
  });
}
async function UploadFileUgu(query) {
  return new Promise(async (results, output) => {
    const GetData = new BodyForm();
    GetData.append("files[]", fs.createReadStream(query));
    await axios({
      ...
      results(urls.data.files[0]);
    }).catch(err => output(err));
  });
}
async function createUrl(query, IsURl = "1") {
  try {
    if (!query) {
      return;
    }
    if (!IsURl || IsURl === "1" || IsURl.toLowerCase() === "telegraph") {
      return await TelegraPh(query);
    }
    if (IsURl === "2" || IsURl.toLowerCase().includes("ugu")) {
      return await UploadFileUgu(query);
    }
  } catch (err) {
    console.log("ERROR IN SCRAPPING FOR CREATE URL()\n", err);
  }
}
function webp2mp4File(query) {
  return new Promise((result, unexcepted) => {
    const DataExc = new BodyForm();
    DataExc.append("new-image-url", "");
    DataExc.append("new-image", fs.createReadStream(query));
    axios({
      method: "post",
      url: "https://s6.ezgif.com/webp-to-mp4",
      data: DataExc,
      headers: {
        "Content-Type": "multipart/form-data; boundary=" + DataExc._boundary
      }
    }).then(({
      data: _0x3dfa4a
    }) => {
      const DataResult = new BodyForm();
      const FileName = cheerio.load(_0x3dfa4a);
      const output = FileName("input[name=\"file\"]").attr("value");
      DataResult.append("file", output);
      DataResult.append("convert", "Convert WebP to MP4!");
      axios({
        method: "post",
        url: "https://ezgif.com/webp-to-mp4/" + output,
        data: DataResult,
        headers: {
          "Content-Type": "multipart/form-data; boundary=" + DataResult._boundary
        }
      }).then(({
        data: results
      }) => {
        const outputs = cheerio.load(results);
        const allData = "https:" + outputs("div#output > p.outfile > video > source").attr("src");
        result({
          status: true,
...
function wikimedia(query) {
  return new Promise((message, _0xd97134) => {
    axios.get("https://commons.wikimedia.org/w/index.php?search=" + query + "&title=Special:MediaSearch&go=Go&type=image").then(results => {
      let find = cheerio.load(results.data);
      let IDBN = [];
      find(".sdms-search-results__list-wrapper > div > a").each(function (_0x2e6c41, lass_) {
        IDBN.push({
          title: find(lass_).find("img").attr("alt"),
          source: find(lass_).attr("href"),
          image: find(lass_).find("img").attr("data-src") || find(lass_).find("img").attr("src")
        });
      });
      message(IDBN);
    });
...
function botpic() {
  return new Promise((msend, _0x596d7c) => {
    let images = getString("global");
    let randomize = ["" + images.pic1, "" + images.pic2, "" + images.pic3, "" + images.pic4, "" + images.pic5, "" + images.pic6];
    const output = randomize[Math.floor(Math.random() * randomize.length)];
    msend(output);
  });
}
